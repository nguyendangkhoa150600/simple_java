...
SELECT DISTINCT ma_mh FROM phu_trach_bo_mon WHERE ma_mh IS NOT NULL OR ma_mh != '';

# d. Mã môn học của những môn học đã từng được tổ chức thi ít nhất 1 lần.
SELECT DISTINCT ma_mh FROM ket_qua_hoc_tap WHERE diem_thi_giua_ky IS NOT NULL OR diem_thi_giua_ky != '';

# e. Mã giáo viên của những giáo viên đã từng làm chủ nhiệm cho ít nhất 1lớp nào đó.
SELECT DISTINCT ma_gvcn FROM lop WHERE ma_lop IS NOT NULL OR ma_lop != '';
